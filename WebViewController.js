var WebViewController = JSB.defineClass(
  'WebViewController : UIViewController',
  {
    viewDidLoad: function () {
      ...
      self.lanCode = 'en';
      self.lastOffset = 0;
      var savedLan = NSUserDefaults.standardUserDefaults().objectForKey(
        'marginnote_deepl_lan'
      );
      ...
      self.webView.loadRequest(
        NSURLRequest.requestWithURL(
          NSURL.URLWithString('http://papago.naver.com/')
        )
      );

      self.lanButton = UIButton.buttonWithType(0);
      self.lanButton.frame = {
        x: webFrame.x + 10,
      ...
      self.lanButton.setTitleColorForState(
      ...
      self.lanButton.backgroundColor =
      ...
      self.lanButton.addTargetActionForControlEvents(
        self,
        'changeLanguage:',
        1 << 6
      );
      ...
      menuController.commandTable = [
        {
          title: 'English',
          object: self,
          selector: 'changeLanguageTo:',
          param: 'en',
          checked: self.lanCode == 'en',
        },
        {
          title: 'Korean',
          object: self,
...
WebViewController.prototype.translateText = function (text) {
  ...
  this.text = text;
  var url =
    'https://papago.naver.com/?sk=auto&tk=' +
    this.lanCode +
    '&hn=1&st=' +
    encodeURIComponent(this.text);
  this.webView.loadRequest(
...
WebViewController.prototype.updateButton = function () {
  if (this.lanCode == 'en') this.lanButton.setTitleForState('English', 0);
  else if (this.lanCode == 'ko') this.lanButton.setTitleForState('Korean', 0);
};
WebViewController.prototype.updateOffset = function () {
  if (!this.webView || !this.webView.window) return;
  // var webControlerler = this;
  // this.webView.evaluateJavaScript(
  //   'document.getElementsByClassName("lmt__side_container--target")[0].getBoundingClientRect().top + document.body.scrollTop+document.documentElement.scrollTop;',
  //   function (ret) {
  //     if (!webControlerler.webView) return;
  //     if (ret && !isNaN(parseFloat(ret))) {
  //       webControlerler.webView.scrollView.contentOffset = {
  //         x: 0,
  //         y: parseFloat(ret) + 0,
  //       };
  //     } else {
  ...
  this.webView.evaluateJavaScript(
    ...
    document.getElementsByClassName("scroll")[0].style.padding = "310px 0px 0px";\
    ',
    function (ret) {}
  );
};
